...
如果把浮点数在数轴上表示，像尺子一样标注刻线，就会发现浮点数并不是等步长的，越靠近0,越密集。

假如仍然按照一开始的方案，即(1+尾数)*2^(指数).最小的数a=2^(-126),但是第二小的数b=2^(-126)+2^(-149)，

$$a-0>>b-a$$

所以规定在指数全为0时，不采用省略1的记录方法，即实际值为
...
例如x=-1.5\*10^38, y=1.5\*10^38,z=1.0
...
IEEE 754对浮点数的舍入要求为四舍六入（刚刚好为5,例如3.5或2.5时，向偶数舍入，变成4和2）